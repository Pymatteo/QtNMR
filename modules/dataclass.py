...
############### END PLOTTERS ###############     
    ...
    def left_shift(self, check):
       if self.__filename and self.__shifter and (self.__ft_flag is False):
         if check is True:
                self.bl_correction(False)
         self.__dataplt = analysis.leftshift(self.__dataplt, self.__shifter,check) 
         self.printer.append('Shifted: '+ str(self.__shifter))
         self.plotstd()
    ...
    def exp_apodization(self):
       if self.__filename and (self.__ft_flag is False):
          ...
          LB = 100
          self.__dataplt = analysis.exp_apodization(self.__dataplt, self.__time[1],LB) 
          self.plotstd()

    def sqr_apodization(self):
       if self.__filename and self.__shifter and (self.__ft_flag is False):
          self.bl_correction(False)
          self.__dataplt = analysis.sqr_apodization(self.__dataplt, self.__shifter) 
          self.plotstd()
